...
xd3_decode_bytes (xd3_stream *stream, uint8_t *buf, usize_t *pos, usize_t size)
{
      ...
      if (stream->avail_in == 0)
  {
    ...
    return XD3_INPUT;
      ...
      want = size - *pos;
      take = std::min (want, stream->avail_in);
    ...
    usize_t          size)
{
      ...
      if (output->next == output->avail)
  {
    ...
    output = (*outputp) = aoutput;
      ...
      take =std::min (output->avail - output->next, size);
...
xd3_iopt_flush_instructions (xd3_stream *stream, int force)
{
      ...
      else
  {
    ...
    average = gap / 2;
    newsize =std::min (MAX_MATCH_SPLIT, gap - average);
...
xd3_encode_buffer_leftover (xd3_stream *stream)
{
  ...
  take =std::min (room, stream->avail_in);
...
{
  usize_t ipos = 0;
  usize_t n =std::min(stream->winsize, input_size);
  ...
  case XD3_INPUT: {
    n =std::min(stream->winsize, input_size - ipos);
    if (n == 0) {
      goto done;
    }
    xd3_avail_input (stream, input + ipos, n);
  ...
  if (is_encode)
    {
      ...
      config.winsize =std::min(input_size, (usize_t) XD3_DEFAULT_WINSIZE);
      config.iopt_size =std::min(input_size / 32, XD3_DEFAULT_IOPT_SIZE);
      config.iopt_size =std::max(config.iopt_size, 128U);
...
xd3_srcwin_setup (xd3_stream *stream)
{
   ...
   * now.  */
  src->srcbase = stream->match_minaddr;
  src->srclen  = std::max ((usize_t) length,
...
xd3_source_extend_match (xd3_stream *stream)
{
      ...
      while (stream->match_back < stream->match_maxback)
  {
    ...
    tryrem =std::min (tryoff, stream->match_maxback - stream->match_back);
      ...
      if ((ret = xd3_getblk (stream, tryblk)))
  {
    ...
    return ret;
      ...
      tryrem =std::min(stream->match_maxfwd - stream->match_fwd,
       src->onblk - tryoff);
...
xd3_srcwin_move_point (xd3_stream *stream, usize_t *next_move_point)
{
   ...
   * maximum window size. */
  logical_input_cksum_pos =std::min((stream->total_in + stream->input_position) * 2,
        (stream->total_in + stream->input_position) +
...
XD3_TEMPLATE(xd3_string_match_) (xd3_stream *stream)
{
 ...
 restartloop:
  ...
  if (xd3_iopt_last_matched (stream) > stream->input_position)
    {
      stream->min_match = std::max(MIN_MATCH,
